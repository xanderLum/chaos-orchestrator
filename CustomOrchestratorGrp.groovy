def call() {
    ...
    stage('Inject DB MySQL Server in endpoint javaapp with 100% CPU and 90% Memory parallel for 35 minutes') {
        try {
            ...
            //injectCPUUtil Params(
            //                  String endpointName, def timeoutInMilliseconds, String id, String injectionHomeDir,
    ...
    stage('Inject DB MySQL Server in endpoint javaapp with 3GB DiskIO and 90% DiskSpace parallel for 35minutes seconds') {
        ...
        try {
            ...
            echo "executing ${chaosUtil.injectDiskSpaceFault(this, "javaapp", 2.1e+6, "javaapp", null, "dbgrp-iteration1-diskSpace", "/home/trps.mas.gov.sg/a_osp_chaos", 90, false, null, 1, "schedId-1", "schedulingTest")}"
